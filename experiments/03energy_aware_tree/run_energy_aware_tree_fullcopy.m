...
%% ============ 可追溯输出目录（Run ID + Tag） ============
% 你可以：setenv('GNC_TAG','v02_tree') 来标记本次实验版本
tag = getenv('GNC_TAG');
if isempty(tag), tag = 'tree_energy_static'; end
    [run_dir, meta] = make_run_dir('tree_energy_static', 'Tag', tag, 'Script', mfilename('fullpath'));
root_outdir = run_dir;
...
for m = 1:numel(route_modes)
    ...
    for vi = 1:numel(values)
        ...
        if exist('apply_energy_aware_cfg','file')==2
            ...
            if strcmpi(mode,'tree_energy')
                % Force STATIC energy-aware tree for strict comparability with topo_tree baseline
            ...
            out = main_energy_aware_tree(p);
            f = out.final;
% ---- Field mapping for compatibility with topo_tree plotting ----
% main_energy_aware_tree may output different field names; map to the
% canonical names used in run_topo_tree: avg_delay, avg_hops, alive_ratio, energy_total.
if isfield(f,'avg_delay')
    delay_val = f.avg_delay;
elseif isfield(f,'Delay')
    delay_val = f.Delay;
else
    delay_val = NaN;
end

if isfield(f,'avg_hops')
    hops_val = f.avg_hops;
elseif isfield(f,'Hops')
    hops_val = f.Hops;
else
    hops_val = NaN;
end

if isfield(f,'alive_ratio')
    alive_val = f.alive_ratio;
elseif isfield(f,'Alive')
    alive_val = f.Alive;
else
    alive_val = NaN;
end

if isfield(f,'energy_total')
    energy_val = f.energy_total;
elseif isfield(f,'Etotal')
    energy_val = f.Etotal;
elseif isfield(f,'Esum')
    energy_val = f.Esum;
else
    energy_val = NaN;
end
            ...
            PDR_arr(r)    = f.PDR;
            delay_arr(r)  = delay_val;
            hops_arr(r)   = hops_val;
            alive_arr(r)  = alive_val;
            energy_arr(r) = energy_val;
