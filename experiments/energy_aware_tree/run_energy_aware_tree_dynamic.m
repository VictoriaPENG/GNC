function run_energy_aware_tree_dynamic()
% ============================================================
% Dynamic energy-aware tree experiment (separate from topo_tree baseline)
% ------------------------------------------------------------
% Purpose:
%   Study impact of periodic tree rebuilding (tree_rebuild_period) on:
%     - PDR / delay / hops
%     - lifetime (FND/HND/LND)
% Under each topo_mode.
%
% How to compare with other experiments:
%   1) Use the SAME base config + same sweeps (lambda_list/alpha_list/N_list/Rc_list)
%   2) Only change route_modes to include:
%        - tree (baseline static tree)
%        - tree_energy_static (energy-aware, rebuild_period=inf)
%        - tree_energy_dyn50 (energy-aware, rebuild_period=50)
%        - tree_energy_dyn20 (energy-aware, rebuild_period=20)
%   3) Keep seeds and n_runs identical.
%
% Output:
%   results/<run_id>/dynamic/<topo_mode>/... (via make_run_dir)
% ============================================================

clc; close all;

% You can tag runs from env:
tag = getenv('GNC_TAG');
if isempty(tag), tag = 'tree_energy_dynamic'; end

% Reuse the baseline runner but override route modes by mapping tags -> configs
run_energy_aware_tree_dynamic_impl(tag);

end

function run_energy_aware_tree_dynamic_impl(tag)
% ---- Copy of run_energy_aware_tree_fullcopy main body, but with dynamic variants ----
% This file assumes you have:
%   - run_topo_tree style dependencies on path
%   - main_energy_aware_tree.m
%   - make_run_dir.m, save_run_metadata.m, paper_plot_style.m, plot_metric_vs_param_paper.m

S = paper_plot_style();

route_modes = {'tree', 'tree_energy_static', 'tree_energy_dyn50', 'tree_energy_dyn20'};
topo_modes = {'uniform','cluster','road'};

n_runs = 20;
base_seed = 1;

% ---- Base config (match run_topo_tree as closely as possible) ----
base = struct();
base.verbose = true;
base.seed = base_seed;
...
% Topology extras (match run_topo_tree defaults)
...
% Energy-aware parameters (kept constant here; you can sweep them separately)
base.tree_energy_beta = 2.0;
base.tree_energy_min_frac = 0.10;

% Sweeps (same as topo_tree)
lambda_list = [0.01 0.02 0.05 0.08 0.10];

% Output
[run_dir, meta] = make_run_dir('tree_energy_dynamic', 'Tag', tag, 'Script', mfilename('fullpath'));
root_outdir = fullfile(run_dir, 'dynamic');
...
for tm = 1:numel(topo_modes)
    ...
    % Sweep lambda
    res_lambda = sweep_param_parallel_dynamic(route_modes, n_runs, base_seed, base_this, 'lambda', lambda_list);
    ...
    save(fullfile(outdir,'results_dynamic_tree.mat'), 'route_modes','topo_mode','n_runs','base_this','lambda_list','res_lambda');
end
...
fprintf("ALL DONE. Dynamic outputs saved under: %s\n", root_outdir);

end

function res = sweep_param_parallel_dynamic(route_modes, n_runs, base_seed, base, field, values)
% Similar to run_topo_tree sweep, but interprets route_modes as variants.
...
for m = 1:numel(route_modes)
    ...
    for vi = 1:numel(values)
        ...
        % Map variant to actual routing mode + rebuild period
        if strcmpi(mode,'tree')
            p.route_mode = 'tree';
            p.tree_energy_aware = false;
            p.tree_rebuild_period = inf;
        else
            p.route_mode = 'tree_energy';
            p.tree_energy_aware = true;
            if strcmpi(mode,'tree_energy_static')
                p.tree_rebuild_period = inf;
            elseif strcmpi(mode,'tree_energy_dyn50')
                p.tree_rebuild_period = 50;
            elseif strcmpi(mode,'tree_energy_dyn20')
                p.tree_rebuild_period = 20;
            else
                p.tree_rebuild_period = 50;
            end
        end
