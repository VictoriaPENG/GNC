...
tag = getenv('GNC_TAG');
if isempty(tag), tag = 'tree'; end
[run_dir, meta] = make_run_dir('energy_aware_tree', 'Tag', tag, 'Script', mfilename('fullpath'));
root_outdir = run_dir;
...
for m = 1:numel(route_modes)
    ...
    p.route_mode = route_modes{m};
        % Mode-specific tweaks (Energy-aware Tree)
...
% Enable energy-aware tree building only for 'tree_energy' mode.
if isfield(p,'route_mode') && strcmpi(string(p.route_mode), "tree_energy")
    p.tree_energy_aware = true;
    % If user did not set rebuild period, default to 50 to track energy dynamics
    if ~isfield(p,'tree_rebuild_period') || isempty(p.tree_rebuild_period) || isinf(p.tree_rebuild_period)
        p.tree_rebuild_period = 50;
    end
else
    % Do not force off if user explicitly sets tree_energy_aware for other modes,
    % but default behavior is off.
    if ~isfield(p,'tree_energy_aware')
