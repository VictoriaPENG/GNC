function out = main_energy_tree(p)
% ============================================================
...
%   - 选项：可通过 tree_energy_bias>0 在建树时偏向剩余能量较高的父节点。
...
E_for_tree = E;
parent = build_converge_tree(Adj_tree, exp_prr, bs_idx, p.tree_cost, E_for_tree, p.tree_energy_bias);
...
for t = 1:p.T
    ...
    if startsWith(lower(p.route_mode), 'tree') && isfinite(p.tree_rebuild_period)
        if mod(t-1, max(1,round(p.tree_rebuild_period))) == 0
            ...
            E_tree = E;
            E_tree(~alive) = 0;
            parent = build_converge_tree(Adj_tree, exp_prr, bs_idx, p.tree_cost, E_tree, p.tree_energy_bias);
        end
    end
...
function parent = build_converge_tree(Adj, exp_prr, bs_idx, tree_cost, E, energy_bias)
...
use_energy = ~use_hop && energy_bias > 0;
if ~exist('E','var') || isempty(E)
    E = ones(n,1);
end
if ~exist('energy_bias','var') || isempty(energy_bias)
    energy_bias = 0;
end
E = E(:);
finite_mask = isfinite(E);
if any(finite_mask)
    E(finite_mask) = E(finite_mask) / max(max(E(finite_mask)), 1e-9);
end
E(~finite_mask) = 1; % 基站或非法值不参与能量偏好
...
for iter = 1:n
    ...
    if use_hop
        ...
        w = 1 ./ max(p_, 1e-6); % ETX
        if use_energy
            % energy_bias 越大，越偏好高能节点作为汇聚树父节点
            e_weight = (1 - energy_bias) + energy_bias ./ max(E(nbrs), 1e-6);
            w = w .* e_weight;
        end
    end
...
%   - 'tree_energy'       : 与 tree 相同，但建树时引入能量偏好（tree_energy_bias）
% 说明：
%   - 如果 parent(i) 不可用（不连通/死亡），会自动回退到 etx 选路，保证不崩。
def.tree_cost = 'etx';                 % 'etx' | 'hop'
def.tree_energy_bias = 0;              % 0 表示不考虑能量；(0,1] 越大越偏向高剩余能量节点
